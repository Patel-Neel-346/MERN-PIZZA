...
describe('AUTH ROUTES', () => {
    ...
    describe('POST /auth/register', () => {
        ...
        it('should return access and refresh tokens in cookies', async () => {
            ...
            const accessToken = cookies.find(c => c.startsWith('accessToken='))?.split('=')[1]?.split(';')[0];
            const refreshToken = cookies.find(c => c.startsWith('refreshToken='))?.split('=')[1]?.split(';')[0];
        ...
        it('should store refresh token in database', async () => {
            ...
            const res = await request(app).post('/auth/register').send(userData);
            const refreshTokens = await connection.getRepository(RefreshToken).find({
                where: { user: { id: res.body.user.id } },
                relations: ['user'],
            });
    ...
    describe('POST /auth/logout', () => {
        it('should logout user and clear tokens', async () => {
            ...
            expect(res.statusCode).toBe(200);
            expect(res.body.message).toBe('User has been SuccesFully log out');
        });
    });
});